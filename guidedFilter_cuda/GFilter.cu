...
{
    cudaCheckError(cudaMalloc((void **)&d_tempA_, sizeof(float) * wid * hei));
    cudaCheckError(cudaMalloc((void **)&d_tempB_, sizeof(float) * wid * hei));
    cudaCheckError(cudaMalloc((void **)&d_tempC_, sizeof(float) * wid * hei));
    cudaCheckError(cudaMalloc((void **)&d_tempD_, sizeof(float) * wid * hei));
}
...
void GFilter::guidedfilterTest(float *imgOut, float *imgInI, float *imgInP, int wid, int hei, int rad, double eps)
{
    ...
    cudaCheckError(cudaMalloc((void **)&d_imgInI, sizeof(float) * wid * hei));
    cudaCheckError(cudaMalloc((void **)&d_imgInP, sizeof(float) * wid * hei));
    cudaCheckError(cudaMalloc((void **)&d_imgOut, sizeof(float) * wid * hei));
    cudaCheckError(cudaMemset(d_imgOut, 0, sizeof(float) * wid * hei));

    cudaCheckError(cudaMemcpy(d_imgInI, imgInI, sizeof(float) * wid * hei, cudaMemcpyHostToDevice));
    cudaCheckError(cudaMemcpy(d_imgInP, imgInP, sizeof(float) * wid * hei, cudaMemcpyHostToDevice));
