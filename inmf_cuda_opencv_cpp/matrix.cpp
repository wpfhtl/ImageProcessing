...
// add by myself : smher 2017-04-13
__global__ void  mat_abs_cal(matrix& a);
...
void mat_abs_d(matrix &a)
{
	dim3 block(16, 16);
	dim3 grid;
	grid.x = (a.dim[0] + 15) / block.x;
	grid.y = (a.dim[1] + 15) / block.y;
	//cudaError_t cudaStat = cudaSuccess;
	if(a.mat_d == NULL && a.mat == NULL)
	{
		std::cout << "no data in abs calculation!" << std::endl;
	}
	else if(a.mat_d == NULL && a.mat != NULL)
	{
		//cudaStat = copy_matrix_to_device(&a);
		copy_matrix_to_device(&a);
		//assert(cudaStat == cudaSuccess);
		mat_abs_cal<<<grid, block>>>(a);
	}
	else				// a.mat_d != NULL
		mat_abs_cal<<<grid, block>>>(a);
}

// col vector a is stored on !!  host !!  and have been reset be zero!!!
// you should malloc correct address before call this function
// return matrix a, input data : data ...  All operators are performed on host !!!
void mat_Restore(matrix &a, float *data, int m)
{
	int row = a.dim[0];
	for(int i = 0; i < m; ++i)
	{
		a.mat[i + i*row] = data[i];
	}
}

// reshape the matrix to correct size
void mat_Reshape(matrix &a,float *data, int rowA, int colA, int colData)
{
	for(int i = 0; i < rowA; ++i)
	{
		...
		{
			a.mat[i + j*colA] = data[i + j * colData];
		}
	}
}
